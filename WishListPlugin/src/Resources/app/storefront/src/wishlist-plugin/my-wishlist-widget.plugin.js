...
export default class MyWishlistWidgetPlugin extends Plugin {
    init() {
     ...
     */
    _getWishlistStorage() {
        this._wishlistStorage = window.PluginManager.getPluginInstanceFromElement(this.el, 'MyWishlistStorage');
    }
    /**
     * @private
     */
     ...
     */
     _registerEvents() {
        this.$emitter.subscribe('Wishlist/onProductsLoaded', () => {
            ...
            // });
            window.PluginManager.getPluginInstances('CheckProductAddedWishlist').forEach((pluginInstance) => {
                pluginInstance.initStateClasses();
            });
        });
